...
public class Intermedio extends JPanel implements ActionListener {
    ...
    private int casillasFaltantes = 0, bomba = 20;
...
//                    System.out.println(minutos + ":" + segundos);
    ...
    public Intermedio(int idUsuario) {
        ...
        botonPartida.setBounds(186, 5, 30, 30);
    ...
    public void nuevaPartidaIntermedio() {
        ...
        MostrarTodo(false);
        colocarMinasIntermedio();
        contornoBombaIntermedio();
        mostrarCasillasIntermedio();
        eventoIntermedio();
    ...
    private void inicializaContador() {
        ...
        contador.setBounds(290, 5, 30, 30);
    ...
    public void colocarMinasIntermedio() {
        for (int f = 0; f < bomba; f++) {
            for (int c = 0; c < bomba; c++) {
    ...
    public void contornoBombaIntermedio() {
        for (int f = 0; f < bomba; f++) {
            for (int c = 0; c < bomba; c++) {
    ...
    public void mensajeIntermedio(int f, int c) throws LineUnavailableException, IOException, UnsupportedAudioFileException {
        if (f >= 0 && f < bomba && c >= 0 && c < bomba && casillas[f][c] == false) {
            if (casillas[f][c] == false) {
                casillas[f][c] = true;
                if (BotonesInferiores[f][c] == 9) {
                    hilo.suspend();//se pausa el hilo
                    contadorInactivo = true;//el hilo esta pausado sonidoExplosion();
                    MostrarTodo(true);
                    sonidoPerdedor();
                    botonPartida.setIcon(imagenesPartidaNueva[1]);
                    consultas.AgregarPuntuacion(idUsuario, "Intermedio", casillasFaltantes);
                    ...
                    puntuacion.setText(casillasFaltantes + "/360");
                }
            }
            if (BotonesInferiores[f][c] == 0) {
                mensajeIntermedio(f, c - 1);
                mensajeIntermedio(f, c + 1);
                mensajeIntermedio(f - 1, c);
                mensajeIntermedio(f + 1, c);
            }
        }
    }
    ...
    public void pulsarBotonIntermedio(int f, int c) throws LineUnavailableException, IOException, UnsupportedAudioFileException {
        mensajeIntermedio(f, c);
        mostrarCasillasIntermedio();
    }

    public void eventoIntermedio() {
        for (int f = 0; f < bomba; f++) {
            for (int c = 0; c < bomba; c++) {
    ...
    public void mostrarCasillasIntermedio() {
        for (int f = 0; f < bomba; f++) {
            for (int c = 0; c < bomba; c++) {
                if (casillas[f][c] == false) {
